...
def find_min_empty_room(number, full):
    if number not in full:
        full[number] = number + 1
        return number
    else:
        new_number = find_min_empty_room(full[number], full)
        full[number] = new_number + 1
        return new_number

def solution(i, room_number):
    full = {}
    answer = []
    for i in room_number:
        answer.append(find_min_empty_room(i, full))
    return answer